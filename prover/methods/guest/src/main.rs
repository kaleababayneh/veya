...
//! DER) a `u32` length followed by the raw bytes via `read_slice`, then a serde-encoded
//! `(row_index, offer_id)`. Bulk `read_slice` is far cheaper than deserializing a `Vec<u8>`
...
fn main() {
    ...
    let (row_index, offer_id): (u32, u64) = env::read();
    let input = ProverInput { eml, dkim_pubkey_der, row_index, offer_id };
