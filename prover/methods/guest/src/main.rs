...
//! DER) a `u32` length followed by the raw bytes via `read_slice`, then serde-encoded `offer_id` and the
//! attachment hint (host-located, guest-verified offsets of the e-dekont part). Bulk `read_slice` is far
...
fn main() {
    ...
    let input = ProverInput { eml, dkim_pubkey_der, offer_id, attachment };
