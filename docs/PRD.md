...
| Why now | (1) **FAST** makes every TRY bank transfer instant, 24/7, with IBAN + name in the record. (2) **Ziraat Bankası** e-mails a 30-day statement (`Hesap Hareketleri`) that is **DKIM-signed** (`ileti.ziraatbank.com.tr`, RSA-1024/SHA-256) and lists every FAST/EFT row with date, fiş no, counterparty IBAN and amount. (3) **Stellar Protocol 25+** exposes native BN254 pairing on Soroban; our verifier already verifies an RISC Zero Groth16 receipt on testnet for ~27.4M CPU insns. |
...
| **Buyer** (Mehmet) | TRY in a **Ziraat** account (v1: payer must bank with Ziraat), Freighter wallet | Crypto without a CEX, no trust in a stranger | "Reserve an offer, pay by FAST, prove it from my own statement e-mail, receive the crypto automatically." |
...
|---|---|---|
| LOCK_SECS | **3600 s (60 min)** | Buyer path: FAST (≤1 min) → trigger statement e-mail (arrives in ~2 min; measured 20:02→20:04) → download .eml (1 min) → proof (2–10 min) → tx. ~15 min typical; 60 min absorbs bank/app hiccups while not freezing seller liquidity for hours. Not extendable in v1. |
...
| Currency | statement row currency must be TRY | Ziraat TRY account only in v1 |
...
| Date window | `payment_date` (dd.mm.yyyy from statement, Istanbul local) must satisfy **istanbul_date(locked_at) ≤ payment_date ≤ istanbul_date(ledger_now)**, Istanbul = UTC+3 fixed (no DST since 2016) | statement has no time-of-day; payments before the lock are not accepted (prevents recycling an unrelated earlier payment) |
| Payee match | `sha256(normalize(IBAN))` from proof == `offer.iban_hash`; normalize = uppercase, strip spaces | statement rows print IBAN without spaces |
...
| What leaves the browser | the **entire .eml** (30-day statement incl. balances, all counterparties) goes to the prover. |
...
| On-chain footprint | only: offer_id, buyer, hashes (dkim key, domain, payee IBAN), amount_kurus, payment_date, nullifier. Never the statement or payer IBAN. |
