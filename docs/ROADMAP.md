...
| 21 | Bank-provider abstraction, second bank | **Open (post-hackathon); VakıfBank is the candidate.** Sample from 12 Sep (`testdata/private/vakif-incoming-fast.eml`): DKIM-signed by `vakifbank.com.tr` (s=smtp2, rsa-sha256, relaxed, no `l=`, RSA-1024) with the receipt as a Flate-compressed PDF whose text is plain `Tj` runs with ASCII labels (TUTARI, AÇIKLAMASI, SORGU REFERANS NO, ALICI HESAP NO). Needs: no_std inflate + text-operator scan in the guest, a Vakıf outgoing-layout parser (outgoing sample also in hand: `vakif-outgoing-fast.eml` — `FAST Giden Ödeme`, date-time, `TUTARI`, `MASRAF TUTARI`, sender name, **full** recipient IBAN, `SORGU NO`, `AÇIKLAMASI <typed text>`; recipient name field looks empty in the plain runs), provider id in the journal, per-provider domain/key allow-list in the escrow. ~2–3 days. |
