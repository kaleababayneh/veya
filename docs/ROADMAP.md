...
| 3a | Anchor integration (hackathon requirement; DevRel's TR mock anchor) | **Done 2026-09-13.** zkOTC is a SEP client (SEP-1 discovery, SEP-10 wallet login, SEP-6 deposit/withdraw, SEP-38 quotes) of `tr-mock-anchor.fly.dev`, positioned as the makers' licensed rail: `/anchor` funds USDC inventory and cashes out; the market shows the anchor's rate as the price to beat and each USDC ad's discount to it; SEP-1 `stellar.toml` published. A USDC ad (merve, ₺47.90) was funded through the anchor. Real anchor later = home-domain change. |
