...
| `prover` | **RISC Zero** guest (`zkotc-guest`, image id `0x13f87d48…f333`), `zkotc` CLI (image-id / execute / prove), `zkotc-server` | 33.7M cycles on a real statement |
...
1. **x86 Linux box** (16+ vCPU, ≥16 GB, optionally an NVIDIA GPU with `--features cuda`): `rzup install risc0-groth16`, then run `zkotc-server`. One statement ≈ 34M cycles → minutes on CPU, ~1 min on a GPU, plus the Groth16 wrap.
